...
(function () {
    ...
    const darkMode = () => {
        if (document.querySelector('body').classList.contains('auto')) {
            const backgroundsToBeUpdated = Array.from(document.querySelectorAll('.modal-content, .preloader-inner'));
            const overlay = document.querySelector('.overlay');
            const preloader = Array.from(document.querySelectorAll('.preloader'));
            backgroundsToBeUpdated.push(overlay);
            if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
                document.querySelector('body').classList.add('dark')
                // Loop through the modal content elements and assign updated background color
                backgroundsToBeUpdated.forEach((element, index) => {
                    element.style.backgroundColor = "#222738";
                });
                preloader.forEach((element, index) => {
                    element.style.backgroundColor = "rgba(34, 39, 56, 0.8)"
                });
            } else {
    ...
    const updateMode = () => {
        ...
        // manually update elements
        switch (colorMode) {
            case 'dark':
                // update backgrounds
                $('.modal-content, .preloader-inner, .overlay').css('background-color', '#222738');
                // update preloaders
                $('.preloader').css('background-color', 'rgba(34, 39, 56, 0.8)');
                // update form-select borders
                $('.form-select').css('border-color', '#30384f');
                // update modal header & footer border color
                $('.modal-header').css('border-bottom-color', '#30384f');
                $('.modal-footer').css('border-top-color', '#30384f');
                break;

            case 'light':
                // update backgrounds
                $('.modal-content, .preloader-inner').css('background-color', '#ffffff');
                // update preloaders
                $('.preloader').css('background-color', 'rgba(255, 255, 255, 0.8)');
                // update form-select borders
                $('.form-select').css('border-color', '#eee');
                // update modal header & footer border color
                $('.modal-header').css('border-bottom-color', 'rgb(238, 238, 238)');
                $('.modal-footer').css('border-top-color', 'rgb(238, 238, 238)');
                break;
    ...
    Array.from(colorMode, (mode) => {
        mode.addEventListener('click', (e) => {
            ...
            // manually update elements
            switch (mode.getAttribute('data-value')) {
                case 'dark':
                    // update backgrounds
                    $('.modal-content, .preloader-inner, .overlay').css('background-color', '#222738');
                    // update preloaders
